...
│   └── 11_data_augmentation.ipynb          # 数据增强 — 对抗过拟合（含 GPU 批量增强优化）
├── utils/
│   ├── early_stopping.py                    # EarlyStopping 共享工具类
│   ├── augmentation.py                      # 数据增强（CPU + GPU 批量）共享工具类
...
| 2d | `11_data_augmentation.ipynb` | 数据增强防过拟合（含 GPU 批量增强优化） | 旋转/平移/缩放/擦除，CPU vs GPU 管线对照 |
...
| 11 | `11_data_augmentation.ipynb` | 无（独立运行，含 MLP+CNN 对照实验 + GPU 批量增强） |
...
│           ↓                                                     │
│  Step 2d: 11_data_augmentation ← 数据增强对抗过拟合（GPU批量优化）       │
│           ↓                                                     │
...
| 增强方法 | 参数范围 | 效果 |
|---------|---------|------|
| `RandomAffine` (旋转+平移+缩放) | 旋转 ±10°, 平移 ±10%, 缩放 90%~110% | 合并为一次重采样，高效 |
...
`11_data_augmentation.ipynb` 提供了 **两种增强管线**：

| 管线 | 增强位置 | GPU 利用率 | 适用场景 |
|------|----------|-----------|----------|
| **CPU 增强**（传统） | DataLoader worker 逐样本做 torchvision transforms | 低（GPU 空等 CPU） | 兼容性最好 |
| **GPU 批量增强**（推荐） | `GPUBatchAugment` 在 GPU 上对整个 batch 做变换 | 高（GPU 全速运行） | **推荐** |

**GPU 增强的核心优化**：
1. **旋转+平移+缩放合并为一次 `F.grid_sample`** — 省 2 次图像重采样
2. **增强在 GPU 上批量执行** — GPU 算图像变换比 CPU 快 10-50x
3. **搭配 AMP 混合精度 + torch.compile** — 额外 1.5-2x 加速

### 对照实验预期

`11_data_augmentation.ipynb` 中做了 MLP + CNN 的对照实验：

| 指标 | 无增强 | CPU 增强 | GPU 增强 |
|------|--------|---------|----------|
| Train Accuracy | ~99.9% | ~99.2%（更难"背诵"） | 同 CPU |
| Test Accuracy | ~98.6% | ~98.9%（泛化更好） | 同 CPU |
| Train-Test Gap | ~1.3% | ~0.3%（过拟合显著减轻） | 同 CPU |
| GPU 利用率 | 低 | 低（CPU 瓶颈） | **高** |
| 训练速度 | 基准 | 比无增强慢 | **比 CPU 增强快 2-4x** |
...
# 1. DataLoader 不做增强，只加载原始图像
train_loader = DataLoader(
    TensorDataset(X_train_img, y_train_t),
    batch_size=128, shuffle=True,
    num_workers=4, pin_memory=True,
    persistent_workers=True, prefetch_factor=4   # 性能优化参数
)
